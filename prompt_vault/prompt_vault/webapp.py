...
def launch_gui(db_path: Path | None = None) -> int:
    try:
        ...
        print("请先安装依赖：pip install -r prompt_vault/requirements-gui.txt")
        print("推荐（Windows + conda）：prompt_vault/scripts/install_gui_deps.bat")
        return 2
...
def _create_api_app(db_path: Path | None = None):
    ...
    dev_url = os.environ.get("PROMPT_VAULT_DEV_URL", "").strip() or None
    return create_app(db_path=path, frontend_dist=frontend_dist, dev_url=dev_url)
...
def _enable_high_dpi() -> None:
    ...
    try:
        ...
        # Best effort only; GUI still works without this optimization.
        return
